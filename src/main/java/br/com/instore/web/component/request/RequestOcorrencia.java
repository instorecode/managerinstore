...
public class RequestOcorrencia implements java.io.Serializable {
    ...
    public void salvar(OcorrenciaBean bean, OcorrenciaUsuarioBean[] ocorrenciaUsuarioBeanList) {
        try {
            ...
            repository.query("delete from ocorrencia_usuario where ocorrencia = " + bean.getId()).executeSQLCommand();
            if (null != ocorrenciaUsuarioBeanList && ocorrenciaUsuarioBeanList.length > 0) {
                for (OcorrenciaUsuarioBean oub : ocorrenciaUsuarioBeanList) {
                    if (oub != null) {
                        oub.setOcorrenciaBean(bean);
                        repository.save(oub);
                    }
                }
            }
