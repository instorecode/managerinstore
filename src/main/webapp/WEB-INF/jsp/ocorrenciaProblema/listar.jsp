...
<instore:template isGrid="false">
    <jsp:body> 
        ...
        <c:forEach items="${funcionalidadeBeanList}" var="func">
            <c:if test="${func.mappingId eq '/ocorrencia-problema/cadastrar'}">
                <c:set scope="session" var="form_access" value="${true}"></c:set>
            </c:if>

            <c:if test="${func.mappingId eq '/ocorrencia-problema/atualizar/{id}'}">
                <c:set scope="session" var="update_access" value="${true}"></c:set>
            </c:if>

            <c:if test="${func.mappingId eq '/ocorrencia-problema/remover/{id}'}">
            ...
            <div class="block">
                <div class="content">
                    <c:if test="${form_access}">
                        ...
                        <h2>Cadastro</h2>
                        <hr />
                        <form name="FORM_CADASTRO" method="POST" data-formtable="true" action="${url}/ocorrencia-problema/cadastrar">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Descrição</label>
                                        <input type="text" name="ocorrenciaProblemaBean.descricao" class="form-control" placeholder="Descrição"  
                                               data-rule-required="true" 
                                               data-rule-minlength="3" 
                                               data-rule-maxlength="255" 
                                               value="${ocorrenciaProblemaBean.descricao}">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="edit">
            <c:if test="${update_access}">
                ...
                <form name="FORM_ALTERAR_[[__PK__]]" method="POST" data-formtable="true" action="${url}/ocorrencia-problema/cadastrar">
                    <input type="hidden" name="ocorrenciaProblemaBean.id" value="${ocorrenciaProblemaBean.id}" field="id" />
        ...
        <div class="delete">
            <c:if test="${delete_access}">
                ...
                <form name="FORM_DELETAR_[[__PK__]]" method="POST" data-formtable="true" action="${url}/ocorrencia-problema/remover/[[__PK__]]">
                    <input type="hidden" name="ocorrenciaProblemaBean.id" value="[[__PK__]]" />
